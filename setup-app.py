...
def get_cert(appname, domain, certname):
    ...
    """
    command = "bx --quiet cf ssh %s -c \"cat ~/app/conf/live/%s/%s\"" % (
        appname, domain, certname)
...
for entry in settings['domains']:
    ...
    for host in entry['hosts']:
        if host == '.':
            call(["bx", "cf", "map-route", appname, domain,
                  "--path", ".well-known/acme-challenge/"])
        else:
            call(["bx", "cf", "map-route", appname, domain, "--hostname",
                  host, "--path", ".well-known/acme-challenge/"])
